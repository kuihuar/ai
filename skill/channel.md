...
func (wp *WorkerPool) processTask(task interface{}, workerID int) interface{} {
    // 模拟任务处理
    time.Sleep(time.Millisecond * 100)
...
| 类型 | 用途 | 示例 |
|------|------|------|
| `chan struct{}` | 信号传递 | `done := make(chan struct{})` |
| `chan bool` | 状态传递 | `status := make(chan bool)` |
| `chan int` | 计数和限流 | `semaphore := make(chan int, 5)` |
| `chan string` | 字符串传递 | `messages := make(chan string)` |
| `chan interface{}` | 任意类型传递 | `data := make(chan interface{})` |
| `chan<- int` | 只发送 | `sendCh := (chan<- int)(ch)` |
| `<-chan int` | 只接收 | `recvCh := (<-chan int)(ch)` |

## 总结

Channel 是 Go 并发编程的核心，它提供了：
...
1. **goroutine 间通信** - 传递数据和信号
2. **同步控制** - 协调多个 goroutine
3. **并发模式** - 工作池、扇入、扇出、管道等
4. **错误处理** - 错误传播和结果处理
5. **资源管理** - 对象池、限流等

### 最佳实践

1. **正确关闭 channel** - 只有发送者应该关闭
2. **使用 select 避免阻塞** - 提供非阻塞操作
3. **合理使用缓冲** - 提高性能但避免过度缓冲
4. **结合 context** - 提供取消和超时机制
